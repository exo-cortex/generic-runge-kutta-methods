...
// double pendulum code adapted from equations here:
...
impl System for DynamicalSystem {
    ...
    fn f(i: &Self::StateT, p: &Self::ParamT) -> Self::StateT {
        ...
        let h1 =
            i.p1 * i.p2 * phi_diff.sin() / p.l1 * p.l2 * (p.m1 + p.m2 * phi_diff.sin().powi(2));

        let h2 = (p.m2 * p.l2.powi(2) * i.p1.powi(2) + (p.m1 + p.m2) * p.l1.powi(2) * i.p2.powi(2)
            - 2.0 * p.m2 * p.l1 * p.l2 * i.p1 * i.p2 * phi_diff.cos())
            / (2.0 * p.l1.powi(2) * p.l2.powi(2) * (p.m1 + p.m2 * phi_diff.sin().powi(2)).powi(2));

        DoublePendulumState {
            phi1: (p.l2 * i.p1 - p.l1 * i.p2 * phi_diff.cos())
                / (p.l1.powi(2) * p.l2 * (p.m1 + p.m2 * phi_diff.sin().powi(2))), // check
            p1: -(p.m1 + p.m2) * p.g * p.l1 * i.phi1.sin() - h1 + h2 * (2.0 * phi_diff).sin(), // check
            phi2: (-p.m2 * p.l2 * i.p1 * phi_diff.cos() + (p.m1 + p.m2) * p.l1 * i.p2) / p.m2
                * p.l1
                * p.l2.powi(2)
                * (p.m1 + p.m2 * phi_diff.sin().powi(2)),
            p2: -p.m2 * p.g * p.l2 * i.phi2.sin() + h1 - h2 * (2.0 * phi_diff).sin(),
        }
    }
}
